#!/usr/bin/env python3
# warp_filter_ref.py — the AV1 Warped_Filters[193][8] table (spec 7.11.3.5),
# transcribed DETERMINISTICALLY from dav1d src/tables.c dav1d_mc_warp_filter and
# anchored by its MD5. dav1d stores it SPEC-LITERAL (positive center, rows sum +128)
# UNLIKE its resize (negated) / subpel (halved) neighbours. Signed int8 values.
# The oracle for drishti av1_warp_filter; the MD5 assert is the trust anchor.
import hashlib
...
MD5 = "c764ff07aecee5aba2348072f285059b"

def _bytes():
    return bytes((v & 0xFF) for r in WARP_FILTER for v in r)

def checksum():
    ck = 0
    for i, r in enumerate(WARP_FILTER):
        for t, v in enumerate(r):
            ck = (ck + ((i * 8 + t + 1) * v)) & 0x7FFFFFFFFFFFFFFF
    return ck

def main():
    assert len(WARP_FILTER) == 193
    assert hashlib.md5(_bytes()).hexdigest() == MD5, "MD5 anchor failed"
    assert all(sum(r) == 128 for r in WARP_FILTER), "row-sum"
    assert all(WARP_FILTER[i] == WARP_FILTER[192 - i][::-1] for i in range(1, 192)), "symmetry"
    assert WARP_FILTER[192] == WARP_FILTER[191], "sentinel"
    print("rows=193 sum-per-row=128 MD5=%s checksum=%d" % (MD5, checksum()))
    for i in (0, 1, 32, 63, 64, 96, 127, 128, 160, 191, 192):
        print("  row %3d = %s" % (i, WARP_FILTER[i]))
