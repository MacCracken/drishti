...
## Status — 0.7.128 (AV1: a profile-0 KEYFRAME decoder verified against libaom — **all EIGHT published conformance vectors in the gate decode their keyframe BIT-EXACTLY** vs libaom's own reference MD5s (`make conformance` = 25 matched / 2 known-gap / 0 regressed), including 128x128 superblocks (libaom's default). 0.7.128 closed the last two published gaps — E2d, a heap overflow in `av1_clear_cdef` that corrupted the decoder's own CDF blob, not the spec misreading two sessions had hunted — and E2e, a CfL bottom-edge chroma defect on any frame whose luma height/8 is odd, which the published corpus structurally could not see because every published height is a multiple of 16. MINIMAL inter frames also decode end-to-end from real bytes with WITNESSED motion: single-ref forward-only (0.7.119), a realistic order-hint header and a COMPOUND / backward-reference frame (0.7.120), sign-bias MV negation witnessed to pixels, cross-frame CDF inheritance 7.20/7.21 (0.7.121), an intra block inside an inter frame decoding to PIXELS (0.7.122-0.7.124), and SEGMENTATION reaching the dequant on both lanes (0.7.125). Inter frames still DIVERGE from the reference (E2b/E2c) and the deblocker filters every inter block at the wrong level (D3). The "encoder" is a plan-driven bitstream writer, not an encoder. See docs/development/roadmap.md "HONEST STATUS" for the phased, checkable remainder — no "% done" is printed)

The bitstream/container/header layer of every family is built, spec-
derived, and adversarially tested (30,288 suite assertions + 7,410 fuzz
