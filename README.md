...
## Status — 0.7.129 (AV1: **`make conformance` is 33 matched / 0 KNOWN-GAP / 0 regressed** — every case in the gate is BIT-EXACT against libaom, keyframes and INTER frames alike. All eight published conformance vectors, the generated corpus with and without loop filters, and the committed reproducers all match `aomdec` byte for byte. 0.7.129 got there by fixing five defects that had been mis-filed for releases as "inter reconstruction rounding" and "inter entropy desync": the temporal-MV scan had NEVER RUN (`av1_mvctx_set_use_ref_frame_mvs` had zero callers); the deblocker filtered every block of every frame at the INTRA level; saved CDF bundles carried their adaptation counters across frame boundaries; `warpEstimation`'s least-squares constants were wrong (`LS_STEP` 2 instead of 8) — as was the Python reference port that generated its known-answers, so 43 assertions had agreed with a wrong decoder; and `LoopfilterTxSizes` was stamped uniformly across var-tx blocks. 8/10/12-bit, multi-tile, 64x64 and 128x128 superblocks, superres, segmentation, compound/warp/OBMC/inter-intra. The "encoder" remains a plan-driven bitstream writer, not an encoder. See docs/development/roadmap.md for the batched release plan through 0.7.142 — no "% done" is printed)

The bitstream/container/header layer of every family is built, spec-
derived, and adversarially tested (30,300 suite assertions + 7,410 fuzz
