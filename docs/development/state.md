...
**0.7.79** — cut 2026-07-14, not yet tagged (user's git). **Inter prediction — the WARP-SAMPLE LEAVES (spec
7.10.4 find_warp_samples / add_sample + has_overlappable_candidates).** read_motion_mode (5.11.27) gates on
both: OBMC needs an overlappable neighbour, LOCALWARP needs NumSamples > 0. `src/av1_mv.cyr` adds
**av1_has_overlappable_candidates** (is there an inter block above/left? checked at 8x8 granularity — the
`+= 2` step with an `x4 | 1` ODD-column probe — and clipped to the FRAME, not the tile),
**av1_warp_add_sample** (7.10.4.2: skipped unless inside / written this frame / sharing RefFrame[0] /
single-ref; snaps to the candidate BLOCK's top-left, takes the block centre, validates the MV delta against
Clip3(16,112,Max(Block_Width,Block_Height)); an INVALID sample is kept only if it is the FIRST scanned — the
spec's "if no small motion vectors are found, return the first large one"), **av1_find_warp_samples**
(7.10.4.1: the above row + left column — one sample if the neighbour is at least as large, else stepping
across smaller ones — then the top-left/top-right corners + the NumSamples=1 special case), and the
Av1WarpSamples record (CandList[8][4] + NumSamples + NumSamplesScanned). Verified against a NEW committed
spec-literal port (`scripts/refs/warp_samples_ref.py`), with the fixture grid laid by the **REAL
av1_mi_store_mode** rather than poked by hand — which MATTERS: add_sample reads Mvs at the candidate block's
TOP-LEFT, a cell that only exists if the whole footprint was written. My first hand-built fixture got this
wrong and reported 4 valid samples for MVs 900 apart; the tell was a candidate whose source and destination
were identical. ALL MUTATION-VERIFIED (0 at baseline): compound test inverted → 12 failures; candRow snap
removed → 3; first-large special case removed → 2; NumSamples=1 case removed → 1; overlappable `>`→`>=` → 1;
the 8x8 step 2→1 → 1; the threshold's 112 clamp→999 → 1; top-right `w4`→`h4` → 2. **THREE MUTATIONS INITIALLY
SURVIVED, ALL FROM ONE CAUSE — a UNIFORM, SQUARE fixture** (the 0.7.76/0.7.77 aliasing lesson again): a 16x16
block has w4==h4 so the top-right probe's w4 was indistinguishable from h4; it never reaches the 112 clamp;
and at an EVEN MiCol the `x4|1` probe maps both x and x+1 to the same column, so the 8x8 step is
unobservable. Closed with a NON-SQUARE block (BLOCK_8X16) asserting the TOP-RIGHT candidate slot
specifically, a 128x128 block with an MV delta of 120 (strictly between the clamped 112 and an unclamped
128), and an ODD MiCol case where only column 9 carries an inter block. Also guarded every MiSizes read
before it indexes the 22-entry Num_4x4_Blocks_* tables, and documented that the spec's `& ~(candH4-1)` is
transcribed as `& (0 - candH4)` — equal in two's complement, verified exhaustively. Next: read_motion_mode's
full 5.11.27 gating (these leaves + is_scaled), then read_ref_frames + the inter_block_mode_info
orchestrator, inter_frame_mode_info, and the inter tile decode. **Prior: the gating orchestrators
...
- `make test` — 37 suites / **26,242 assertions**: drishti 51 · bits
