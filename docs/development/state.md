...
**0.7.53** — cut 2026-07-12, not yet tagged (user's git). **Superres upscaling — row
driver (spec 7.16), reference-confirmed against dav1d.** `av1_superres_upscale_row`
upscales one plane row: a single subpel accumulator starts at `mx0 − (1<<14)` (dav1d's
`src_x = −1`) and steps by `dx`, one `av1_superres_filter_pixel` per output. This is the
combined-accumulator form of dav1d's `resize_c` (`src/mc_tmpl.c`, user-provided); a Python
port (`scratchpad/resize_ref.py`) proves it identical to dav1d's split `src_x`/`mx` form
and the cyrius output matches `resize_c` byte-for-byte (2× / 1.5× / flat). Building it
surfaced a latent kernel fix — the integer base now uses an **arithmetic** shift (`>>>`),
since the accumulator is negative at a row's start (positive-only known-answers unchanged).
The row driver's `dx`/`mx0` are inputs; their per-plane geometry (dav1d `get_upscale_x0`)
is the next bite, then the per-plane loop + pipeline wiring. **Prior: superres kernel +
`Upscale_Filter` table 0.7.52 (verified incl. a per-phase position checksum vs an
independent dav1d transcription); multi-tile track complete 0.7.47–0.7.51; 10-bit 0.7.46.**
The remaining distance to
1.0 is superres (geometry+wiring) + inter + conformance +
...
|--------|--------|---------|
| `src/drishti.cyr` | core `dr_` | error record + code bands, `drishti_version()` → 753, format sniff |
...
| `src/av1_superres.cyr` | `av1_` | superres upscaling (7.16) — Upscale_Filter[64][8] (dav1d resize filter negated to spec form; row-sum/integer-pel/mirror + per-phase position-checksum verified) + av1_superres_filter_pixel (one sample: phase/base/edge-clamp + Round2(sum,7) + Clip1) + **av1_superres_upscale_row** (the row loop, == dav1d resize_c, reference-tested). dx/mx0 geometry + pipeline wiring are later bites |
...
- `make test` — 30 suites / **20,723 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 148 · av1_lr 80 · av1_decode 184 · h264 326 · h265 276 · vpx 287
