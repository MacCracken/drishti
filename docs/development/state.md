...
**0.7.78** — cut 2026-07-14, not yet tagged (user's git). **Inter prediction — the GATING ORCHESTRATORS
(spec 5.11.28 read_interintra_mode + the 5.11.23 interp_filter tail).** 0.7.71/0.7.72 landed these symbols'
LEAF reads and left the gating that decides whether each is coded "the caller's concern" — this is that
caller, and the first bite of the inter_block_mode_info arc. **av1_read_interintra_mode** (the full 5.11.28:
the gate is !skip_mode && enable_interintra_compound && !isCompound && BLOCK_8X8<=MiSize<=BLOCK_32X32; outside
it interintra=0 and NOTHING is read; inside it composes the four leaf reads + surfaces the spec's block-level
side-effects — RefFrame[1]=INTRA_FRAME, AngleDeltaY/UV=0, use_filter_intra=0 — through an Av1InterIntraRec,
and forces wedge_sign=0). **av1_needs_interp_filter** (skip_mode / LOCALWARP suppress; a LARGE block in
GLOBALMV/GLOBAL_GLOBALMV needs a filter only when the global model is a plain TRANSLATION — a warp model
carries its own filtering). **av1_read_interp_filters** (non-SWITCHABLE → both dirs take the frame filter,
nothing coded; SWITCHABLE → code enable_dual_filter?2:1 dirs each gated by needs_interp_filter, an ungated dir
is EIGHTTAP, and without dual filter dir1 MIRRORS dir0). Paired encoder-inverses. Verified by 75 new
assertions: every gated round-trip writes a **MARKER symbol after the gated block**, so a reader/writer
disagreement about how many symbols a CLOSED gate consumes fails loudly instead of silently desyncing; all
five gate-closing conditions + both MiSize bounds are covered (BLOCK_8X8 and BLOCK_32X32 are IN the gate), as
are needs_interp_filter's skip/LOCALWARP/both global modes/either-GmType-TRANSLATION/the >=8 large boundary
(incl. a SMALL block where the large guard means GLOBALMV still needs a filter), and the loop's
non-switchable / switchable+dual / switchable+mirror / both suppression paths. MUTATION-VERIFIED (0 at
baseline): interintra gate `>`→`>=` → 3; isCompound inverted → 18; wedge_sign forced 1 → 2; LOCALWARP→OBMC →
1; TRANSLATION→IDENTITY → 2; large `>=8`→`>8` → 1; dual mirror inverted → 3; needs gate inverted → 13.
**TWO REAL DEFECTS FIXED:** (1) a DUPLICATE FUNCTION NAME — the new driver collided with 0.7.72's leaf
av1_read_interintra_mode, and cyrius SILENTLY SHADOWS duplicates (last-def-wins, warn-only), so this would
have swapped one for the other with no error; the leaf is now av1_read_interintra_mode_sym (matching the
av1_read_compound_type_sym precedent: leaf = _sym, driver = plain). (2) a STALE deferral comment claiming the
compound-reference CDF contexts are "a caller input here" — untrue since 0.7.76 — surfaced by `make lint`
flagging it as an untracked deferral; the comment was WRONG, not merely unreferenced. Next: read_motion_mode's
full gating (5.11.27) needs find_warp_samples (7.10.4) + has_overlappable_candidates — its own bite; then
read_ref_frames + the inter_block_mode_info orchestrator, inter_frame_mode_info, and the inter tile decode.
**Prior: the last three CDF contexts `av1_intermode.cyr` 0.7.77 (EVERY inter context is now derived);
...
- `make test` — 37 suites / **26,205 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,491 · av1_intermode 3,515 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
