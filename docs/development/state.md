...
**0.7.82** — cut 2026-07-16, not yet tagged (user's git). **Inter prediction — INTER_BLOCK_MODE_INFO
...
- `make test` — 37 suites / **26,842 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,528 · av1_intermode 4,115 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
...
>
> **Where we are (0.7.82):** keyframes decode end-to-end. **Inter frames do NOT** — the
> tile decoder is still intra-only. The ENTIRE per-block inter mode-info decode now
> exists as one call: `av1_inter_block_mode_info` (5.11.23, 0.7.82) composes everything
> from 0.7.62–0.7.81 — MC driver + DPB, the `find_mv_stack` arc, every symbol read,
> MI-grid population, every CDF context, warp samples, the `read_motion_mode` gate, the
> `read_ref_frames` dispatcher. Two layers remain to a decoded inter frame.
>
> **Do these in order** (each is one bite; read
> [`docs/guides/verification.md`](../guides/verification.md) first):
>
> 1. **`inter_frame_mode_info`** (5.11.15) — the outer dispatch (segment id, skip_mode,
>    skip, `is_inter` → inter block mode info or the intra path), and its wiring of the
>    5.11.15 preamble (`av1_nbctx_setup`) that today's tests do by hand.
> 2. **The inter tile decode** — wire the above + `compute_prediction` (the MC driver)
>    into `decode_block`/`decode_tile`, so `av1_decode_stream` decodes a real inter
>    frame. **This is the milestone.**
> 3. Then: the temporal scan (7.10.2.5/6 — needs the DPB to save MVs, currently a pixel-
