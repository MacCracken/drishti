...
**0.7.80** — cut 2026-07-16, not yet tagged (user's git). **Inter prediction — READ_MOTION_MODE (spec
5.11.27), the full gating driver + is_scaled.** The last gating orchestrator. `src/av1_frame.cyr` adds
**av1_is_scaled** (5.11.27: the ref's RefUpscaledWidth / RefFrameHeight vs the current FrameWidth /
FrameHeight as 14-bit ROUNDED ratios, `REF_SCALE_SHIFT = 14`; the width/height asymmetry is the spec's —
superres never changes height; hostile refFrame / ref_frame_idx-slot / zero-dim inputs report "scaled",
never read OOB). `src/av1_intermode.cyr` adds the **av1_read_motion_mode** DRIVER — early SIMPLE with NO
symbol on skip_mode / !is_motion_mode_switchable / min(w,h)<8 / a GLOBALMV-family block whose global model
is beyond TRANSLATION (only tested when !force_integer_mv) / compound / interintra (RefFrame[1] ==
INTRA_FRAME) / no overlappable inter neighbour; then find_warp_samples ALWAYS runs (NumSamples also feeds
warp_estimation later), and `force_integer_mv || NumSamples == 0 || !allow_warped_motion || is_scaled`
picks @@use_obmc (OBMC/SIMPLE) over @@motion_mode — plus the paired **av1_write_motion_mode** inverse. The
0.7.71 leaves took the `_sym` suffix (leaf = _sym, driver = plain name). Verified against a NEW committed
spec-literal port (`scripts/refs/motion_mode_ref.py`: branch decision + is_scaled). Tests drive every gate
BOTH ways behind an 8-bit 0xA5 LITERAL marker, plus decode-side CONFORMANCE cases hand-built with the LEAF
writers — which is what catches a gate bug SHARED by driver + inverse (proven: a paired is_scaled drop
survives every round-trip and dies only there). ALL MUTATION-VERIFIED — **21 mutations, 0 survivors**
(per-gate drops, boundary flips `<8`→`<=8` / `>`→`>=`, wrong-symbol, writer-inverse, the paired drop, and
is_scaled's rounding / field / denominator / both index guards). TWO MUTANTS INITIALLY SURVIVED, both TEST
weaknesses: a 1-symbol binary marker passes a desync by luck (→ the 8-bit literal), and a guard-alias test
planted its y value at the wrong offset (FRAME_H - 8 aliases FRAME_W[7], not UPSCALED_W[7]) so the guard
was unfalsifiable. A third harness accident — first-occurrence replace matching av1_write_compound_type's
identical skip_mode line — exposed a PRE-EXISTING gap: the compound-type WRITER's skip gate has no killing
test (0.7.73 scope; tracked as a spawned follow-up task). Next: **read_ref_frames** (5.11.25), then the
inter_block_mode_info orchestrator (5.11.23), inter_frame_mode_info (5.11.15), and the inter tile decode.
**Prior: the warp-sample leaves `av1_mv.cyr` 0.7.79 (7.10.4 find_warp_samples / add_sample +
has_overlappable_candidates, vs `scripts/refs/warp_samples_ref.py`, 8 mutations all killed after the
uniform-square-fixture lesson); the gating orchestrators
...
| `src/av1_frame.cyr` | `av1_` | uncompressed frame header (5.9.2, all frame types) + ref-frame state machine (Av1FrameHeader / Av1RefState) + av1_is_scaled (5.11.27 — 14-bit rounded ref-vs-frame dim ratios, hostile-guarded) |
...
- `make test` — 37 suites / **26,427 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,528 · av1_intermode 3,700 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
...
>
> **Where we are (0.7.80):** keyframes decode end-to-end. **Inter frames do NOT** — the
...
> bitstream-read layer, the MI-grid population, every CDF context, the warp samples, and
> now the full `read_motion_mode` gate (every 5.11.27 input incl. `is_scaled` is in).
...
>
> 1. **`read_ref_frames`** (5.11.25) — the dispatcher over the single/compound reads
>    (skip_mode pair / segment overrides / comp_mode → single_ref / compound_ref).
> 2. **`inter_block_mode_info`** (5.11.23) — composes ref frames → `find_mv_stack` → the
>    mode reads → `assign_mv` → interintra / motion mode / compound type / interp filter.
>    Pure composition; every part exists (`read_motion_mode` landed 0.7.80).
> 3. **`inter_frame_mode_info`** (5.11.15) — the outer dispatch (segment id, skip_mode,
>    skip, `is_inter` → inter or intra block mode info).
> 4. **The inter tile decode** — wire the above + `compute_prediction` (the MC driver)
>    into `decode_block`/`decode_tile`, so `av1_decode_stream` decodes a real inter
>    frame. **This is the milestone.**
> 5. Then: the temporal scan (7.10.2.5/6 — needs the DPB to save MVs, currently a pixel-
...
the warp samples (0.7.79) and the full read_motion_mode gating driver + is_scaled (0.7.80) — so every inter
mode-info SYMBOL READ is in, the MI grid that feeds the contexts is populated, and the motion-mode gate is
complete. What is NOT in: the remaining orchestrators — read_ref_frames (5.11.25), inter_block_mode_info
(5.11.23), inter_frame_mode_info
