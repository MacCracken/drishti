...
**0.7.9** — cut 2026-07-10, not yet tagged (user's git). The **0.7.x
AV1 arc** continues through the intra still-picture decode milestone:
recursive filter-intra prediction (7.11.2.3 + the `Intra_Filter_Taps`
table) in `av1_intra.cyr`, on top of 0.7.8's directional prediction,
0.7.6's non-directional modes, and 0.7.7's `>>>` adoption. The remaining
...
|--------|--------|---------|
| `src/drishti.cyr` | core `dr_` | error record + code bands, `drishti_version()` → 709, format sniff |
...
| `src/av1_intra.cyr` | `av1_` | intra prediction (spec 7.11.2) — predict_intra + DC/PAETH/SMOOTH×3 + full directional (edge filter/upsample) + recursive filter-intra (7.11.2.3) |
...
- `make test` — 12 suites / **4,050 assertions**: drishti 51 · bits 1,211
  · ivf 889 · frame 73 · av1 185 · av1_frame 140 · av1_symbol 280 ·
  av1_itx 160 · av1_intra 172 · h264 326 · h265 276 · vpx 287
...
- **adversarial spec reviews** — field-by-field cross-checks against the
  ...
  filter-intra 4 slices → all clean)
...
(0.7.7), the full directional intra prediction (0.7.8), and recursive
filter-intra prediction (0.7.9, this cut). Remaining milestone
sub-bites: chroma-from-luma (7.11.5) → coefficient decode (with the
default CDF tables) → the partition/block reconstruction glue that emits
first pixels. Full per-codec arc plan + the audit/freeze arcs in
