...
**0.7.81** — cut 2026-07-16, not yet tagged (user's git). **Inter prediction — READ_REF_FRAMES (spec
...
- `make test` — 37 suites / **26,590 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,528 · av1_intermode 3,863 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
...
>
> **Where we are (0.7.81):** keyframes decode end-to-end. **Inter frames do NOT** — the
> tile decoder is still intra-only. Everything inter *below* `inter_block_mode_info` is
> built: the MC driver + DPB, the whole `find_mv_stack` arc, the complete inter mode-info
> bitstream-read layer, the MI-grid population, every CDF context, the warp samples, the
> full `read_motion_mode` gate (0.7.80), and the `read_ref_frames` dispatcher (0.7.81).
...
>
> 1. **`inter_block_mode_info`** (5.11.23) — composes `read_ref_frames` →
>    `find_mv_stack` → the mode reads → `assign_mv` → interintra / motion mode /
>    compound type / interp filter. Pure composition; every part now exists.
> 2. **`inter_frame_mode_info`** (5.11.15) — the outer dispatch (segment id, skip_mode,
>    skip, `is_inter` → inter or intra block mode info).
> 3. **The inter tile decode** — wire the above + `compute_prediction` (the MC driver)
>    into `decode_block`/`decode_tile`, so `av1_decode_stream` decodes a real inter
>    frame. **This is the milestone.**
> 4. Then: the temporal scan (7.10.2.5/6 — needs the DPB to save MVs, currently a pixel-
...
the warp samples (0.7.79), the full read_motion_mode gating driver + is_scaled (0.7.80) and the
read_ref_frames dispatcher + seg_feature_active (0.7.81) — so every inter mode-info SYMBOL READ is in, the
MI grid that feeds the contexts is populated, and both the motion-mode gate and the reference dispatch are
complete. What is NOT in: the remaining orchestrators — inter_block_mode_info
