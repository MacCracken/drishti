...
**0.7.75** — cut 2026-07-14, not yet tagged (user's git). **Inter prediction — the NEIGHBOUR CDF CONTEXTS
(spec 5.11.15 preamble + §9 CDF selection). THE FIRST UN-DEFERRAL.** Every inter CDF context derives from the
ABOVE/LEFT neighbours' reference frames; those were CALLER INPUTS from 0.7.68 on because the MI grid held no
decoded data — 0.7.74's storage loops changed that, so the derivations land for real. `src/av1_intermode.cyr`
adds **av1_nbctx_setup** (snapshots the eight values Above/LeftRefFrame[0..1], Above/LeftIntra,
Above/LeftSingle; the unavailable defaults are ASYMMETRIC — RefFrame[0]→INTRA_FRAME but RefFrame[1]→NONE, and
both are `<= INTRA_FRAME` tests, which is exactly WHY NONE=-1 works for Single while INTRA_FRAME=0 works for
Intra), **av1_check_backward** / **av1_count_refs** / **av1_ref_count_ctx** (the §9 leaves the single_ref /
comp_ref family will compose), **av1_is_inter_ctx** (now feeds av1_read_is_inter for real) and
**av1_comp_mode_ctx** (now feeds av1_read_comp_mode for real). Verified by 138 new assertions whose expected
values come from a SPEC-LITERAL PYTHON PORT (`scripts/refs/nbctx_ref.py`, committed) transcribed from the spec TEXT, never
from the Cyrius — an INDEPENDENT ground truth, not a restatement of the code under test: the setup preamble
(incl. the unavailable path proving the grid is NOT consulted — those cells hold different values);
check_backward across every ref value with BOTH boundaries (GOLDEN=4 not backward, BWDREF=5 is, ALTREF=7 is,
8 is not); count_refs + ref_count_ctx; 20 branch-covering known answers; a FULL 5184-COMBO ENUMERATION of
every (AvailU, AvailL, aref0, aref1, lref0, lref1) over a ref set spanning NONE/INTRA/forward/backward,
checked against the port's exhaustive digest (sums 2736/10672 + per-value histograms) and proving no context
ever leaves [0,4)/[0,5) — an out-of-range ctx would index the Is_Inter[4]/Comp_Mode[5] blob OOB; and a derived
ctx driving av1_read_is_inter end-to-end. All MUTATION-VERIFIED: is_inter both-intra 3→2 → 5 failures;
comp_mode XOR→OR → 4; unavail aref1 NONE→INTRA → 1; check_backward `>=`→`>` → 10; the Single `<=`→`<` → 6;
ref_count_ctx `<`→`<=` → 2; comp_mode neither-avail 1→0 → 4 (all 0 at baseline). A **3-slice adversarial spec
review** (spec fidelity / range + memory safety / test adequacy, each finding adversarially verified, run
WORKTREE-ISOLATED so its mutation testing cannot race the main tree — and it HELD: the pre/post checksums show
the agents never touched src/tests, unlike 0.7.73/0.7.74) returned **NO confirmed defects** (4 findings, all
refuted, with unusually strong refutations). Two refuted findings still earned changes: the Av1NbCtx record's
offsets were unpinned (the 0.7.73 circularity class — setup stores and accessors read through the same AV1NB_*
symbols), so test_nbctx_layout now pins them absolutely + pins the last field's end to AV1NB_SIZE (mutation-
verified: LSINGLE overlapping ASINGLE → 23 failures, SIZE 80→72 → 1, aref list-stride 8→16 → 7); and the
AboveSingle/LeftSingle mirror branches needed their check_backward terms covered on BOTH sides (7 cases added).
The reviewer REFUTED the "missing oracle" finding by re-deriving all 11 digest constants from the spec in ~5
minutes without the Python port — a fair correction: the port is a transcription, the SPEC is the oracle. The
port is committed at `scripts/refs/nbctx_ref.py` anyway (cheap regeneration + explicit provenance), which
surfaced a real systemic doc issue: 8 reference ports cited across CHANGELOG/docs/sources.md live only in a
wiped scratchpad — dead citations, flagged for the 0.7.x doc-claim audit. Next: the single_ref / comp_ref ctx family
(derivable now via count_refs), then the contexts needing new grid fields (CompGroupIdxs / CompoundIdxs /
InterpFilters), then the inter tile decode. **Prior: MI-grid population `av1_mv.cyr` 0.7.74 (closes the
...
on the inter track: the rest of the neighbour CDF contexts (the single_ref/comp_ref family, then the ones
needing CompGroupIdxs/CompoundIdxs/InterpFilters added to the grid), then the inter tile decode that lets
...
- `make test` — 37 suites / **25,971 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,465 · av1_intermode 3,307 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
