...
**0.7.77** — cut 2026-07-14, not yet tagged (user's git). **Inter prediction — the LAST THREE CDF CONTEXTS
(spec §9) + the grid fields they need. EVERY INTER CONTEXT IS NOW DERIVED.** comp_group_idx / compound_idx /
interp_filter outlived the other un-deferrals because they read per-cell state the MI record did not carry.
**Av1MiRec grew 80→112** (CompGroupIdxs / CompoundIdxs / InterpFilters[0..1]) and **av1_mi_store_mode** now
writes them exactly where spec 5.11.4 loop 1 does — CompGroupIdxs/CompoundIdxs inside `if (!use_intrabc)`
inside `if (is_inter)`, but InterpFilters inside `if (is_inter)` and OUTSIDE the intrabc guard (an intrabc
block is inter with no compound signalling). **Av1NbCtx grew 80→144**, caching those neighbour values in
av1_nbctx_setup (the spec re-reads the grid at ctx time; caching keeps every ctx a pure function of the
record and is equivalent since every read is Avail-guarded). Then: **av1_comp_group_idx_ctx** (a COMPOUND
neighbour contributes its own comp_group_idx, a SINGLE one contributes 3 only when its RefFrame[0] is ALTREF;
clamped to 5), **av1_compound_idx_ctx** (same shape but the ALTREF bump is **1 not 3**, and there is **NO
clamp** — the spec relies on the max falling out at 5 = 3+1+1; `fwd_eq_bck`, the order-hint distance
comparison, stays a caller input as FRAME state), and **av1_interp_filter_ctx** (a 4-wide bank chosen by
(dir, is-compound), offset by the neighbours' agreed filter type, where a neighbour counts only if it shares
this block's RefFrame[0] in EITHER of its own ref slots and 3 means "no usable neighbour"). Verified against
the committed spec-literal port (`scripts/refs/nbctx_ref.py`, extended): the cache (incl. the unavailable path
proving the cell is NOT consulted); 10+6+13 branch-covering known answers — the 3+3=6→CLAMPED-to-5 case, the
no-clamp max-5 case, all four interp bank bases, sharing via the neighbour's SECOND ref slot, every rung of
the resolution ladder; a **314,928-combo enumeration** matched to the port's digests + proving no ctx leaves
[0,6); the derived ctxs driving the real reads end-to-end; and the storage fields across the whole footprint,
the use_intrabc gate, the intra case, dir independence. ALL MUTATION-VERIFIED (0 at baseline): drop Min(5)→3
failures; comp_group_idx ALTREF 3→1 → 4; compound_idx base 3→1 → 4; its ALTREF 1→3 → 4; interp base *4→*2 → 4;
a wrong ladder rung → 2; drop the ref1-slot share → 1; defeat the use_intrabc gate → 2. test_nbctx_layout also
caught the Av1NbCtx growth ITSELF (its "last field ends at AV1NB_SIZE" assert failed until updated) — exactly
what it exists for. A **3-slice adversarial spec review** (spec fidelity / storage + range safety / test
adequacy), run worktree-isolated WITH THE UNCOMMITTED SLICE COPIED IN + verified present — the fix for
0.7.76's wasted slices, and it WORKED: the agents saw the real code and returned **6 findings, 4 CONFIRMED**,
the first review to confirm more than one. Every confirmation was a TEST gap my own mutation testing missed
because my mutations were not adversarial enough, and each is now fixed and re-verified:
(1) **CompGroupIdxs/CompoundIdxs were aliased everywhere** — every ctx case, the 314,928-combo enumeration,
every store call site AND the reference port itself passed the two equal, so reading the WRONG plane was
invisible in BOTH readers and the writer. Fixed by POISONING the field each context must not read (1-v) and
de-aliasing the store payloads: the reader swaps now fail 5/2 and the store-payload swap 8 (all 0 before).
Note the enumeration digest CANNOT catch this even de-aliased — sweeping both planes over {0,1} makes the sum
invariant — so the poisoned known answers carry it.
(2) **InterpFilters[0]/[1] were aliased** in every ctx case, hiding a dir-indexing bug; now the other dir is
poisoned (3-v) → 6 failures.
(3) **ref1 == INTRA_FRAME was never tested** — the ONLY input separating the spec's `RefFrame[1] >
INTRA_FRAME` from `>=`, and it is LIVE: 5.11.28 sets RefFrame[1]=INTRA_FRAME on an interintra block and
interp_filter is read after it, which is exactly WHY the spec writes `>`. The slip picked a wrong 4-wide CDF
bank silently; three vectors added → 3 failures.
(4) an overclaiming test comment ("interp_filter indexes [16]") on an enumeration that never enumerated
interp_filter — narrowed to what the body actually does, per the arc-end doc-claim rule.
A REAL CODE DEFECT was also found and fixed (by me, mid-review): av1_nbctx_setup was NOT idempotent — the
eight cached grid values were written only inside the avail branches, so a REUSED record (which the tile
decode will hold, one per block) let an unavailable neighbour inherit the PREVIOUS block's. Now written
unconditionally; test_nbctx_setup_reuse catches the regression (8 failures without the fix). **EVERY INTER CDF CONTEXT IS NOW DERIVED FROM THE GRID**;
the caller supplies only frame-level state the spec also treats that way (AvailU/AvailL, the order-hint
distances). Next: the **INTER TILE DECODE** — wiring the read layer + contexts + MI grid + MC driver into a
genuine inter frame — then the temporal scan. **Prior: reference-context family `av1_intermode.cyr` 0.7.76;
...
- `make test` — 37 suites / **26,130 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,491 · av1_intermode 3,440 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
...
- **adversarial spec reviews** — field-by-field cross-checks against the
  ...
  **NO findings** — reviewers byte-for-byte diffed all five compound CDF families vs
  ...
  re-measured), and read_compound_type
