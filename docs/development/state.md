...
**0.7.76** — cut 2026-07-14, not yet tagged (user's git). **Inter prediction — the REFERENCE-CONTEXT FAMILY
(spec §9), completing the un-deferral of the reference reads.** Fifteen symbols, but the spec defines only
EIGHT derivations and wires the rest up as ALIASES ("ctx is computed as in the CDF selection process for X").
`src/av1_intermode.cyr` adds the SEVEN ref_count_ctx derivations — **av1_comp_ref_ctx** (LAST+LAST2 vs
LAST3+GOLDEN), **av1_comp_ref_p1_ctx** (LAST vs LAST2), **av1_comp_ref_p2_ctx** (LAST3 vs GOLDEN),
**av1_comp_bwdref_ctx** (BWDREF+ALTREF2 vs ALTREF), **av1_comp_bwdref_p1_ctx** (BWDREF vs ALTREF2),
**av1_single_ref_p1_ctx** (all fwd vs all bwd), **av1_uni_comp_ref_p1_ctx** (LAST2 vs LAST3+GOLDEN) — plus
**av1_is_samedir_ref_pair** + **av1_comp_ref_type_ctx** (the eighth, the only non-count one: a nested block
over aboveCompInter/leftCompInter/aboveUniComp/leftUniComp whose innermost case compares against BWDREF_FRAME
EXACTLY rather than reusing is_samedir_ref_pair — a subtle trap), and the payoff **av1_single_ref_ctxs** /
**av1_comp_ref_ctxs**, which FILL the six-slot refctx + nine-slot Av1CompCtxIdx records av1_read_single_ref
(0.7.68) / av1_read_compound_ref (0.7.69) already take. The aliases are expressed by CALLING the aliased
function, not duplicating its body, so the spec's "same as" is structural and cannot drift. Verified against
the committed spec-literal port (`scripts/refs/nbctx_ref.py`, extended): per-pair known answers isolating each
context's compared frames; an alias check over 81 neighbourhoods; a **26,244-combo exhaustive enumeration**
(every AvailU/AvailL × every ref value) matched to the port's per-context sum digests + proving no ctx ever
leaves its CDF's range; and the derived contexts driving the real reads end-to-end. **TWO REAL TEST DEFECTS
FOUND BY MY OWN MUTATION TESTING AND FIXED:** (1) the exhaustive digest was BLIND to a swapped count pair —
ref_count_ctx(a,b) maps a<b/==/> to 0/1/2, so swapping inverts 0↔2, and FOUR of these contexts have a
SYMMETRIC histogram (comp_ref [6208,13828,6208]; comp_ref_p1/p2/comp_bwdref_p1 [4213,17818,4213]) which leaves
the sum AND the histogram unchanged; comp_ref's and comp_ref_p2's swaps passed the ENTIRE suite until
test_ref_family_direction (driving every count ctx to BOTH 0 and 2, asserting the pair ORDER) landed — all
four swaps now fail (2/2/4/3). (2) the enumeration's ref set {-1,0,1,4,5,7} OMITTED LAST2/LAST3/ALTREF2 —
exactly the frames comp_ref_p1/p2/comp_bwdref_p1/uni_comp_ref_p1 discriminate — so those contexts had
UNREACHABLE values (comp_ref_p1's histogram was [0,3721,1463]); widened to all nine values (5184→26,244
combos), 0.7.75's digests refreshed. A **3-slice adversarial spec review** (spec fidelity + aliases /
comp_ref_type / test adequacy, worktree-isolated) returned **4 findings, 0 confirmed** — but the review was
PARTLY COMPROMISED and that is worth recording: a git worktree checks out a COMMITTED ref and does NOT
inherit uncommitted changes, and every bite here is uncommitted (the user handles git), so worktrees came up
at HEAD=0.7.75 and two slices reported the reviewed code "does not exist in the repo" (true of THEIR tree).
Isolation silently turned those slices into a review of the PREVIOUS release — a failure mode that reads
exactly like a clean pass. It was rescued by the refute pass, not by design: one refute-agent copied the
uncommitted files into its worktree and reproduced the baseline (3357/0) and the comp_ref-swap kill (2
failures) EXACTLY as claimed; another independently re-transcribed all nine family functions from the spec
and swept all 26,244 combos, confirming no OOB and no spec deviation; a third disproved a suspected
forward-reference hazard (av1_comp_ref_ctxs uses AV1CCTX_* declared later) NON-circularly — poisoning all
nine slots with a sentinel and reading back via HARDCODED offsets rather than the symbols — all nine correct.
The comp_ref_type slice (the highest-risk one) got the real code and found nothing. Lesson recorded in
[[review-agents-need-worktree-isolation]]: isolate AND supply the slice, then VERIFY the agents saw it. Next: the contexts needing NEW grid
fields (CompGroupIdxs / CompoundIdxs / InterpFilters → comp_group_idx / compound_idx / interp_filter), then
the inter tile decode. **Prior: neighbour CDF contexts `av1_intermode.cyr` 0.7.75 (the first un-deferral);
...
on the inter track: the last few CDF contexts (comp_group_idx / compound_idx / interp_filter — they need
CompGroupIdxs/CompoundIdxs/InterpFilters added to the grid), then the inter tile decode that lets
...
- `make test` — 37 suites / **26,021 assertions**: drishti 51 · bits
  ...
  av1_cdef 42 · av1_superres 167 · av1_mc 187 · av1_mc_kernel 10 · av1_mc_emu_edge 15 · av1_mc_driver 157 · av1_mv 1,465 · av1_intermode 3,357 · av1_dpb 82 · av1_lr 80 · av1_decode 190 · h264 326 · h265 276 · vpx 287
