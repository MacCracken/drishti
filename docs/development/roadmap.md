...
- **inter + filters** — motion compensation, deblocking, CDEF, loop
  ...
  **kernel stack is reference-confirmed against dav1d**: `Upscale_Filter` table + pixel
  kernel 0.7.52, row driver 0.7.53, `dx`/`mx0` geometry 0.7.54; the per-plane loop +
  pipeline wiring are next). The superres + inter tables + the dav1d `resize_c` /
  `get_upscale_x0` reference (src/tables.c, mc_tmpl.c, decode.c) are in hand. See memory
