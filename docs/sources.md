...
- **AV1 intra prediction** — spec section 7.11.2 is authoritative (`src/av1_intra.cyr`): 7.11.2.1 predict_intra (reference-sample construction + dispatch + writeback), 7.11.2.2 basic/PAETH, 7.11.2.5 DC, 7.11.2.6 smooth (Sm_Weights tables from the additional-tables annex), 7.11.2.4 directional (the pAngle 90/180 copy cases; the angled interpolation + intra edge filter/upsample 7.11.2.7-12, recursive filter-intra 7.11.2.3, and chroma-from-luma 7.11.5 land in later 0.7.x sub-bites). Mode_To_Angle + the intra-mode enum from the tables/semantics. Cross-checked by an adversarial spec review.
