...
- **AV1 intra prediction** — spec section 7.11.2 is authoritative (`src/av1_intra.cyr`): 7.11.2.1 predict_intra (reference-sample construction + dispatch + writeback), 7.11.2.2 basic/PAETH, 7.11.2.4 directional (all four angle quadrants), 7.11.2.5 DC, 7.11.2.6 smooth, and the intra edge filter/upsample machinery 7.11.2.7/9/10/11/12; Dr_Intra_Derivative / Intra_Edge_Kernel / Sm_Weights / Mode_To_Angle from the additional-tables annex; the intra-mode enum from the semantics. The recursive filter-intra (7.11.2.3), chroma-from-luma (7.11.5), and the get_filter_type neighbour-mode computation (7.11.2.8, a caller input for now) land in later 0.7.x sub-bites. Cross-checked by adversarial spec reviews (non-directional in 0.7.6, directional in 0.7.8).
